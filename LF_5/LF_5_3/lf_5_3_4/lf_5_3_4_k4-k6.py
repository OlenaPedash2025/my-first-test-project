...
type Exhibit = dict[str, Any]


def load_data() -> list[Exhibit]:
    if FILE_PATH.exists():
        with open(FILE_PATH, "r", encoding="utf-8") as file:
            try:
                data: list[Exhibit] = js.load(file)
                return data
...
def save_data(exhibits: list[Exhibit]) -> None:
...
def show_exhibit_details(exhibit: Exhibit) -> None:
...
def add_item(exhibits: list[Exhibit]) -> None:
    print("--Add new exhibit--")

    while True:
        exhibit_title: str = input(
    ...
    while True:
        on_display: str = input(
            "Is the item on display? (yes/no): ").strip().lower()
        if on_display in ['yes', 'no']:
            is_on_display: bool = (on_display == 'yes')
            break
    ...
    while True:
        creator: str = input("Enter the creator of the item: ").strip()
        if creator:
            break
        print("Creator cannot be empty. Please try again.")
    new_id: int = max([ex.get("id", 0) for ex in exhibits], default=0) + 1
    new_exhibit: Exhibit = {
        "id": new_id,
...
def list_of_exhibits(exhibits: list[Exhibit]) -> None:
    if not exhibits:
...
def search_exhibit_by_title(exhibits: list[Exhibit]) -> None:
    query: str = str(
        input("Enter the title of the exhibit to search for: ").strip())
    results: list[Exhibit] = [ex for ex in exhibits if query.lower()
...
def delete_item(exhibits: list[Exhibit]) -> None:
    try:
        target_id: int = int(
    ...
    item_to_delete: Optional[Exhibit] = next(
...
def main() -> None:
    inventory: list[Exhibit] = load_data()
    print(
